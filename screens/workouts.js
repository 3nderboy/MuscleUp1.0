...
export default function Workouts({ route, navigation }) {
  ...
  const [imageNameFront, setImageNameFront] = useState(MuskelmenschBilderWorkout['muskelmodellWV']);  //Hook für das MuskelmenschBild von vorne
  const [imageNameBack, setImageNameBack] = useState(MuskelmenschBilderWorkout['muskelmodellWH']);    //Hook für das MuskelmenschBild von hinten
  ...
  const [gerätenButton, setGerätenButton] = useState(dreiButtons['mitgeräten']);                      //Hook für das Bild des ''Mit Geräten''-Button
  const [Workouts, setWorkouts] = useState(alleWorkouts);                                             //Hook für die Workouts, welche angezeigt werden
  ...
  //Filtert die Übungen nach Zuhause/Mit Hanteln/Mit Geräten:
...
//Die Workouts, welche unten am Screen gelistet werden (die Boxen):
