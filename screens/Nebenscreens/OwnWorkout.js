...
export default function OwnWorkout({ navigation }) {
  ...
  const [modalOpen, setModalOpen] = useState(false);                    //Hook für das Öffnen des Modals mit dem Formular
  const [warningModalOpen, setWarningModalOpen] = useState(false);      //Hook für das Öffnen des Warnungs-Modal
  const [Workouts, setWorkouts] = useState([]);                         //Hook für das Anzeigen der selbst erstellten Workouts

  //Speichert die selbst erstellten Workouts mit der Hilfe von AsyncStorage:
